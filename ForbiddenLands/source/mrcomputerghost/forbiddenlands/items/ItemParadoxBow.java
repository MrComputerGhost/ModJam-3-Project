...
public class ItemParadoxBow extends Item {
	...
	public void onPlayerStoppedUsing(ItemStack par1ItemStack, World par2World,
		...
		if (player.inventory.hasItem(Item.arrow.itemID))
			projectileShot = 1;
		else if (player.inventory.hasItem(Item.skull.itemID))
			projectileShot = 2;
		if (projectileShot > 0)
			hasProjectile = true;
		if (flag || player.inventory.hasItem(Item.arrow.itemID)) {
			...
			par2World.playSoundAtEntity(player, "random.bow", 1.0F, 1.0F
	...
	public void registerIcons(IconRegister par1IconRegister) {
		...
		for (int i = 0; i < this.iconArray.length; ++i) {
			this.iconArray[i] = par1IconRegister.registerIcon(this
					.getIconString() + "_" + bowPullIconNameArray[i]);
		}
	}
	...
	public void onUpdate(ItemStack stack, World world, Entity entity, int par4,
		...
		if (!world.isRemote) {
			...
			if (entityplayer.ridingEntity == entityarrow
					&& entityplayer != null)
				;
			{
