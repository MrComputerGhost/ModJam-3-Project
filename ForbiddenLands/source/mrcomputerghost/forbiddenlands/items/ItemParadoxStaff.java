...
public class ItemParadoxStaff extends ItemTool {
	...
	public void registerIcons(IconRegister par1IconRegister) {
		this.itemIcon = par1IconRegister
				.registerIcon("forbiddenlands:paradoxstaff");
	}
	...
	public ItemStack onItemRightClick(ItemStack item, World world,
		...
		if (player.inventory.hasItem(ForbiddenItems.ParadoxDust.itemID)) {
			...
			Vec3 src = world.getWorldVec3Pool().getVecFromPool(
					player.posX + lookdir.xCoord,
					player.posY + lookdir.yCoord + player.getEyeHeight(),
					player.posZ + lookdir.zCoord);
			Vec3 dst = src.addVector(lookdir.xCoord * RANGE, lookdir.yCoord
					* RANGE, lookdir.zCoord * RANGE);

			MovingObjectPosition mop = world.rayTraceBlocks_do_do(src, dst,
					true, true);
			if (mop == null)
				return item;
			...
			// world.spawnParticle("portal", par2 + par6Random.nextFloat(), par3
			// + 1.1F, par4 + par6Random.nextFloat(), mop.hitVec.xCoord,
			// mop.hitVec.yCoord, mop.hitVec.zCoord);
			int sel = RANDOM.nextInt(8);
			if (sel == 1) {
				player.inventory
						.consumeInventoryItem(ForbiddenItems.ParadoxDust.itemID);
			}
			return item;
		}
		return item;
	...
	public ItemStack onItemUse(ItemStack item, World world, EntityPlayer player) {
		...
		Vec3 src = world.getWorldVec3Pool().getVecFromPool(
				...
				player.posY + lookdir.yCoord + player.getEyeHeight(),
				player.posZ + lookdir.zCoord);
		Vec3 dst = src.addVector(lookdir.xCoord * RANGE,
				lookdir.yCoord * RANGE, lookdir.zCoord * RANGE);

		MovingObjectPosition mop = world.rayTraceBlocks_do_do(src, dst, true,
				true);
		if (mop == null)
			return item;
	...
	public boolean getIsRepairable(ItemStack par1ItemStack,
		...
		return ForbiddenItems.ParadoxDust.itemID == par2ItemStack.itemID ? true
