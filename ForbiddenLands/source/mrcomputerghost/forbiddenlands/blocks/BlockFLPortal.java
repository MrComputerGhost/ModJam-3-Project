...
public class BlockFLPortal extends BlockBreakable {
	...
	public boolean tryToCreatePortal(World par1World, int par2, int par3,
		...
		byte b0 = 0;
		byte b1 = 0;
		if (par1World.getBlockId(par2 - 1, par3, par4) == ForbiddenBlocks.CorruptedBark.blockID
				|| par1World.getBlockId(par2 + 1, par3, par4) == ForbiddenBlocks.CorruptedBark.blockID) {
			b0 = 1;
		}
		if (par1World.getBlockId(par2, par3, par4 - 1) == ForbiddenBlocks.CorruptedBark.blockID
				|| par1World.getBlockId(par2, par3, par4 + 1) == ForbiddenBlocks.CorruptedBark.blockID) {
			b1 = 1;
		}
		if (b0 == b1) {
			return false;
	...
	public void onNeighborBlockChange(World par1World, int par2, int par3,
		...
		if (par1World.getBlockId(par2, i1 - 1, par4) != ForbiddenBlocks.CorruptedBark.blockID) {
			...
			if (j1 == 3
					&& par1World.getBlockId(par2, i1 + j1, par4) == ForbiddenBlocks.CorruptedBark.blockID) {
				...
				if (flag && flag1) {
					...
					if ((par1World.getBlockId(par2 + b0, par3, par4 + b1) != ForbiddenBlocks.CorruptedBark.blockID || par1World
							.getBlockId(par2 - b0, par3, par4 - b1) != this.blockID)
							&& (par1World
									.getBlockId(par2 - b0, par3, par4 - b1) != ForbiddenBlocks.CorruptedBark.blockID || par1World
									.getBlockId(par2 + b0, par3, par4 + b1) != this.blockID)) {
						par1World.setBlockToAir(par2, par3, par4);
					}
				}
			...
			if (thePlayer.timeUntilPortal > 0) {
				...
				thePlayer.mcServer
						...
						.transferPlayerToDimension(
								thePlayer,
								ForbiddenDimensions.dimID,
								new ForbiddenTeleporter(
										thePlayer.mcServer
												.worldServerForDimension(ForbiddenDimensions.dimID)));
			} else {
				thePlayer.timeUntilPortal = 10;
				thePlayer.mcServer.getConfigurationManager()
						.transferPlayerToDimension(
								thePlayer,
								0,
								new ForbiddenTeleporter(thePlayer.mcServer
