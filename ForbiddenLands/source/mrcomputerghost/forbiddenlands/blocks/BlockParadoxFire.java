...
public class BlockParadoxFire extends BlockFire {

	/** The chance this block will encourage nearby blocks to catch on fire */
	private int[] chanceToEncourageFire = new int[256];

	/**
	 * This is an array indexed by block ID the larger the number in the array
	 * the more likely a block type will catch fires
	 */
	private int[] abilityToCatchFire = new int[256];
	@SideOnly(Side.CLIENT)
	private Icon[] iconArray;

	protected BlockParadoxFire(int par1) {
		super(par1);
	...
	public void initializeBlock() {
		...
		this.setBurnRate(Block.deadBush.blockID, 1, 1);
		this.setBurnRate(ForbiddenBlocks.CorruptedBark.blockID, 99, 99);
		this.setBurnRate(ForbiddenBlocks.DeathLeaves.blockID, 99, 99);
		this.setBurnRate(ForbiddenBlocks.DeathPlanks.blockID, 99, 99);
		this.setBurnRate(ForbiddenBlocks.DeathWood.blockID, 99, 99);
	}

	/**
	 * Sets the burn rate for a block. The larger abilityToCatchFire the more
	 * easily it will catch. The larger chanceToEncourageFire the faster it will
	 * burn and spread to other blocks. Args: blockID, chanceToEncourageFire,
	 * abilityToCatchFire
	 */
			...
			if (!flag
				...
				if (!flag && !this.canNeighborBurn(par1World, par2, par3, par4)) {
					if (!par1World.doesBlockHaveSolidTopSurface(par2, par3 - 1,
						...
						&& !this.canBlockCatchFire(par1World, par2, par3 - 1,
								par4, UP) && l == 15
	...
	private void tryToCatchBlockOnFire(World par1World, int par2, int par3,
			int par4, int par5, Random par6Random, int par7) {
		tryToCatchBlockOnFire(par1World, par2, par3, par4, par5, par6Random,
				par7, UP);
	}
	...
	private int getChanceOfNeighborsEncouragingFire(World par1World, int par2,
		...
		if (!par1World.isAirBlock(par2, par3, par4)) {
			...
			int l = this.getChanceToEncourageFire(par1World, par2 + 1, par3,
					par4, b0, WEST);
			l = this.getChanceToEncourageFire(par1World, par2 - 1, par3, par4,
					l, EAST);
			l = this.getChanceToEncourageFire(par1World, par2, par3 - 1, par4,
					l, UP);
			l = this.getChanceToEncourageFire(par1World, par2, par3 + 1, par4,
					l, DOWN);
			l = this.getChanceToEncourageFire(par1World, par2, par3, par4 - 1,
					l, SOUTH);
			l = this.getChanceToEncourageFire(par1World, par2, par3, par4 + 1,
					l, NORTH);
			return l;
	 ...
	 * blockAccess, x, y, z Deprecated for a side-sensitive version
	 */
	@Deprecated
	...
	/**
	 * Retrieves a specified block's chance to encourage their neighbors to burn
	 * and if the number is greater than the current number passed in it will
	 * return its number instead of the passed in one. Args: world, x, y, z,
	 * curChanceToEncourageFire Deprecated for a side-sensitive version
	 */
	@Deprecated
	 ...
	 */
	public void onBlockAdded(EntityPlayer player, World par1World, int par2,
		...
		if (par1World.provider.dimensionId == 0) {
			...
			par1World.playSound((double) ((float) par2 + 0.5F),
					...
					(double) ((float) par4 + 0.5F), "mob.horse.zombie.death",
	...
	public void randomDisplayTick(World par1World, int par2, int par3,
		...
		if (!par1World.doesBlockHaveSolidTopSurface(par2, par3 - 1, par4)
				&& !((BlockParadoxFire) ForbiddenBlocks.ParadoxFire)
						.canBlockCatchFire(par1World, par2, par3 - 1, par4, UP)) {
			if (((BlockParadoxFire) ForbiddenBlocks.ParadoxFire)
					.canBlockCatchFire(par1World, par2 - 1, par3, par4, EAST)) {
				for (l = 0; l < 2; ++l) {
					f = (float) par2 + par5Random.nextFloat() * 0.1F;
					f1 = (float) par3 + par5Random.nextFloat();
					f2 = (float) par4 + par5Random.nextFloat();
					par1World.spawnParticle("portal", (double) f, (double) f1,
							(double) f2, 0.0D, 0.0D, 0.0D);
				}
			}

			if (((BlockParadoxFire) ForbiddenBlocks.ParadoxFire)
					.canBlockCatchFire(par1World, par2 + 1, par3, par4, WEST)) {
				for (l = 0; l < 2; ++l) {
			...
			if (((BlockParadoxFire) ForbiddenBlocks.ParadoxFire)
				...
				for (l = 0; l < 2; ++l) {
					...
					par1World.spawnParticle("death_suspend", (double) f,
							(double) f1, (double) f2, 0.0D, 0.0D, 0.0D);
				}
			}
			...
			for (l = 0; l < 3; ++l) {
				...
				f2 = (float) par4 + par5Random.nextFloat();
				par1World.spawnParticle("footsteps", (double) f, (double) f1,
						(double) f2, 0.0D, 0.0D, 0.0D);
			}
		}
	}
	...
	public void registerIcons(IconRegister par1IconRegister) {
		this.iconArray = new Icon[] {
				par1IconRegister.registerIcon(this.getTextureName()
						+ "_layer_0"),
				par1IconRegister.registerIcon(this.getTextureName()
						+ "_layer_1") };
	}
	...
	public void onEntityCollidedWithBlock(World world, int par2, int par3,
			int par4, Entity entity, EntityPlayer player) {
		if ((entity.entityId == player.entityId)) {
			player.attackEntityFrom(null, 20);
		}
