				...
				for (int i1 = -2; i1 <= 2; ++i1) {
					for (int j1 = -1; j1 < 3; ++j1) {
						...
						this.worldServerInstance.setBlock(k1, l1, i2,
								flag ? ForbiddenBlocks.CorruptedBark.blockID
										: 0);
					}
				}
			}
		...
		if (d3 >= 0.0D) {
			if (flag) {
				this.destinationCoordinateCache.add(j1,
						new ForbiddenPortalPosition(this, i, j, k,
			...
			int j2 = -1;
			/** change this block **/
			if (this.worldServerInstance.getBlockId(i - 1, j, k) == ForbiddenBlocks.FBPortal.blockID) {
				j2 = 2;
			}
			/** change this block **/
			if (this.worldServerInstance.getBlockId(i + 1, j, k) == ForbiddenBlocks.FBPortal.blockID) {
			...
			/** change this block **/
			if (this.worldServerInstance.getBlockId(i, j, k - 1) == ForbiddenBlocks.FBPortal.blockID) {
				j2 = 3;
			}
			/** change this block **/
			if (this.worldServerInstance.getBlockId(i, j, k + 1) == ForbiddenBlocks.FBPortal.blockID) {
			...
			if (j2 > -1) {
				...
				boolean flag1 = !this.worldServerInstance.isAirBlock(i + i3
						+ k3, j, k + j3 + l3)
						|| !this.worldServerInstance.isAirBlock(i + i3 + k3,
								j + 1, k + j3 + l3);
				boolean flag2 = !this.worldServerInstance.isAirBlock(i + i3, j,
						k + j3)
						|| !this.worldServerInstance.isAirBlock(i + i3, j + 1,
								k + j3);
				if (flag1 && flag2) {
					...
					int i4 = k - l3;
					d4 -= (double) l3;
					flag1 = !this.worldServerInstance.isAirBlock(k1 + i3 + k3,
							j, i4 + j3 + l3)
							|| !this.worldServerInstance.isAirBlock(k1 + i3
									+ k3, j + 1, i4 + j3 + l3);
					flag2 = !this.worldServerInstance.isAirBlock(k1 + i3, j, i4
							+ j3)
							|| !this.worldServerInstance.isAirBlock(k1 + i3,
									j + 1, i4 + j3);
				}
				...
				par1Entity.rotationYaw = par8 - (float) (k2 * 90)
						+ (float) (j2 * 90);
			} else {
				par1Entity.motionX = par1Entity.motionY = par1Entity.motionZ = 0.0D;
			}
	...
	public boolean makePortal(Entity par1Entity) {
			...
			for (j2 = k - b0; j2 <= k + b0; ++j2) {
				...
				label274: for (k2 = this.worldServerInstance.getActualHeight() - 1; k2 >= 0; --k2) {
					if (this.worldServerInstance.isAirBlock(i2, k2, j2)) {
						while (k2 > 0
								&& this.worldServerInstance.isAirBlock(i2,
										k2 - 1, j2)) {
							--k2;
						}
		...
		if (d0 < 0.0D) {
			for (i2 = i - b0; i2 <= i + b0; ++i2) {
				...
				for (j2 = k - b0; j2 <= k + b0; ++j2) {
					d2 = (double) j2 + 0.5D - par1Entity.posZ;
					label222: for (k2 = this.worldServerInstance
							.getActualHeight() - 1; k2 >= 0; --k2) {
						if (this.worldServerInstance.isAirBlock(i2, k2, j2)) {
							while (k2 > 0
									&& this.worldServerInstance.isAirBlock(i2,
											k2 - 1, j2)) {
		...
		for (k2 = 0; k2 < 4; ++k2) {
			for (i3 = 0; i3 < 4; ++i3) {
				for (l2 = -1; l2 < 4; ++l2) {
					...
					this.worldServerInstance.setBlock(k3, j3, i4,
							flag ? ForbiddenBlocks.CorruptedBark.blockID
									: ForbiddenBlocks.FBPortal.blockID, 0, 2);
				}
			}
			for (i3 = 0; i3 < 4; ++i3) {
				for (l2 = -1; l2 < 4; ++l2) {
