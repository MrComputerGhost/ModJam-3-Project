...
public class TileEntityTombStoneRenderer extends TileEntitySpecialRenderer {
	...
	public void renderTileEntityAt(TileEntity te, double x, double y,	double z, float f) {
        ...
        if(disp != null)
        {
            if (disp.name != null) {
        	int ind = 5;
        	if (disp.name.toLowerCase() == "player" || disp.name.toLowerCase() == "skeleton") {
        	...
        	} if (disp.name == "Grumm" || disp.name == "Dinnerbone") {
        		GL11.glPushMatrix();
                float uf = 0.8F;
                float fu = 0.8F;
                GL11.glScalef(uf, uf, -uf);
                ...
                renderLabel(EnumChatFormatting.DARK_GRAY + disp.name, 0F, (0.25F)*ind, -0.6, block, disp);
                GL11.glPopMatrix();
                ind++;  
        	} if (disp.name == "MrComputerGhost") {
            ...
            GL11.glPushMatrix();
            GL11.glScalef(fu, fu, fu);
            renderLabel(EnumChatFormatting.DARK_GRAY + disp.name, 0F, (-0.17F)*ind, -0.6, block, disp);
            GL11.glPopMatrix();
            ind++;  
            //if (disp.extra != null) {
            /**GL11.glPushMatrix();
